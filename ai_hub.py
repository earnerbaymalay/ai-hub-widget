...
# Best available monospace font on this machine
def _mono_font(size: int = 9) -> tuple:
    ...
    try:
        ...
        families = set()
    for candidate in ("Cascadia Code", "Cascadia Mono", "JetBrains Mono",
...
class AIHub:
    ...
    def _send(self):
        ...
        # Build argv safely: split the base command then append prompt as a
        # single argument — prevents shell injection from user-supplied text.
        base_argv = shlex.split(ai["cmd"])
        argv      = base_argv + [text]
        ...
        def run():
            try:
                ...
                self.root.after(0, lambda: self._log(
                    f"Error: '{ai['cmd']}' not found — is it installed and in PATH?\n", "error"))
